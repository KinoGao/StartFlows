...
- [Canvas] 画布交互与节点体系全面对齐 LibTV：手势契约（选择/框选/平移/缩放）、添加节点菜单全量结构、逐帧拉片节点、智能剪辑与视频节点空态、节点动作条与右键菜单、底部工具栏角色库/历史记录、空画布模板与深浅主题配色。
- [Canvas] LibTV 视觉样式第二轮对齐：添加节点菜单改紧凑单行行，节点选中框/描边去青色改中性对比色，「AI生成」角标移至媒体左上角内侧，空视频节点改播放水印加纯文字入口，composer 发送键改中性对比圆形键并按视口钳制定位，故事板第一步按 LibTV 列级结构重排（资产名高亮 token、操作列收进 ⋯ 菜单、底部添加镜头与下一步引导）。
- [Canvas] 故事板（脚本工作台）1:1 还原 LibTV 视觉与布局：默认全宽分镜表，剧本正文收进顶栏「剧本」按钮切换的左侧面板；分镜单元格改为无边框纯文本样式并随内容自适应行高，资产名与「[旁白]/角色名：」保持主题色 token 高亮，空单元格显示「+」占位；表头去底色，镜号改纯文字；顶栏合并为单行（左侧工具、居中三步进度、右侧完成度与关闭）。
- [Canvas] 清除自建想象功能并对齐 LibTV 批量模型：移除「整组执行」（悬停工具条/分组菜单/Ctrl+Enter 快捷键/Agent 工具全链路，LibTV 无此功能且误点会整组扣费）；脚本节点工具栏「生成分镜/生成视频」改为打开工作台并弹出带门槛清单与逐镜勾选的批量导出确认，不再一键直铺节点或整剧本直生单条视频；分镜派生节点按「分镜图/分镜视频 #镜号·分镜名」命名，上传媒体节点以原文件名命名，参考垫图列表可辨认。
- [Canvas] 修复脚本工作台「准备资产」单个生成资产图点了没反应：全屏工作台遮住了层级更低的 composer 确认卡片，现改为先关工作台再打开确认卡片；同一资产重复生成替换旧设定图节点不再叠加孤儿节点；资产设定图节点按「角色/场景/道具·名称」命名，一键补齐与单个生成同规则。
- [Canvas] 脚本工作台步骤 2/3 对齐 LibTV：资产卡改为直接显示设定图的卡片（未生成显示状态占位），每个资产支持 AI 生成、从画布选图、本地上传三种设定图方式；「合成提示词」改回与步骤 1 同一张分镜表并高亮「最终提示词」列，待生成/查看提示词都可点击打开双轨弹窗，底部一键合成全部提示词。
- [Canvas] 脚本工作台的付费动作不再点了就直接执行：「AI 拆解」与「一键合成全部提示词」先弹确认（说明文本模型计费与覆盖范围，可取消），确认后才调用模型。
- [画布] 修复工作区自动保存间歇性丢编辑：引导/迁移等就绪窗口内被跳过的推送此前不会补推，版本差静默滞留到下一次编辑；现按真实版本差调度补推并在引导成功后立即补推一轮。
- [生成] 修复 Clash 等代理工具 fake-ip DNS（域名被本地解析到 198.18.0.0/15 保留段）导致系统渠道代理把上游请求误判为内网地址、图片/视频生成 400 失败的问题：配置了系统代理时由代理服务器解析真实 DNS，仅放行带 fake-ip 标记且全部应答非公网的可识别场景，真实内网与元数据地址依旧拒绝；同时修复字符串形态 `error` 字段被吞、失败只显示「状态码 400」的问题。
