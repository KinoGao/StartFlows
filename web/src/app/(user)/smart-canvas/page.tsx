...
import type { CanvasExportFile } from "../canvas/export-types";
import { useCanvasStore } from "../canvas/stores/use-canvas-store";
import { useCanvasUiStore } from "../canvas/stores/use-canvas-ui-store";
import { resolveSiteTitle } from "@/lib/site-brand";
import { usePublicSessionStore } from "@/stores/use-public-session-store";
import { useUserStore } from "@/stores/use-user-store";
import { exportCanvasProjects } from "../canvas/utils/canvas-export";
...
export default function CanvasPage() {
    ...
    const enterProject = (id: string) => {
        router.push(`/smart-canvas/${id}${agentQuery}`);
    };
            ...
            <div className="mx-auto flex w-full max-w-7xl flex-col gap-2 px-2 py-2 sm:gap-6 sm:px-6 sm:py-8">
                <header className="flex flex-wrap items-end justify-between gap-2.5 border-b border-border pb-3 sm:gap-4 sm:pb-5">
                    <div>
                        ...
                        <h1 className="mt-1 text-xl font-semibold sm:mt-2 sm:text-2xl">智能画布</h1>
                    </div>
