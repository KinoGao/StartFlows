...
import { useDirectorDeskHost } from "../io/hostBridge";
...
export function CameraPanel() {
  ...
  const [sendingCaptureIds, setSendingCaptureIds] = useState<Set<string>>(() => new Set());
  ...
  const { sendCaptures } = useDirectorDeskHost();
  ...
  const sendCaptureToCanvas = useCallback(
    async (capture: DirectorCameraCapture) => {
      setCaptureError(null);
      setCaptureNotice(null);
      setSendingCaptureIds((current) => new Set(current).add(capture.id));
      try {
        await sendCaptures([{ dataUrl: capture.dataUrl, fileName: `${capture.name}.png` }]);
        setCaptureNotice(`已发送 ${capture.name} 到画布`);
      } catch (error) {
        setCaptureError(error instanceof Error ? error.message : "截图发送失败");
      } finally {
  ...
  const sendAllCapturesToCanvas = useCallback(async () => {
    const captureItems = cameraCaptureGroups.flatMap((group) => group.captures);
    const allCaptureIds = captureItems.map((capture) => capture.id);
    const allCaptures = captureItems.map((capture) => ({
      dataUrl: capture.dataUrl,
      fileName: `${capture.name}.png`,
    }));
    setCaptureError(null);
    setCaptureNotice(null);
    setSendingCaptureIds((current) => new Set([...current, ...allCaptureIds]));
    try {
      await sendCaptures(allCaptures);
      setCaptureNotice(`已发送 ${allCaptures.length} 张截图到画布`);
    } catch (error) {
      setCaptureError(error instanceof Error ? error.message : "截图发送失败");
    } finally {
      setSendingCaptureIds((current) => {
        const next = new Set(current);
        allCaptureIds.forEach((captureId) => next.delete(captureId));
        return next;
      });
    }
              ...
              >
                ...
                >
                  ...
                  <button
                    ...
                    className="camera-capture-action"
                    disabled={sendingCaptureIds.has(capture.id)}
                    type="button"
                    onClick={(event) => {
                      event.stopPropagation();
    ...
    return (
      <div className="camera-capture-overview-footer">
        ...
        <button
          ...
          disabled={sendingCaptureIds.size > 0}
          type="button"
      ...
      {activeTab === "properties" ? (
        <>
          ...
          <InspectorSection title="相机截图" className="camera-capture-section">
            ...
            {captureError ? <p className="camera-capture-feedback is-error">{captureError}</p> : null}
            {captureNotice ? <p className="camera-capture-feedback is-success">{captureNotice}</p> : null}
